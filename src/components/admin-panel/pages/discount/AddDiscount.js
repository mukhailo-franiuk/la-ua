...
import { useAddDiscountMutation } from "../../../../store/discountSlice/discountSlice";
...
const AddDiscount = (props) => {
    ...
    const addDiscountForm = async (data) =>{
        await addDiscount({
            name:data.name,
            title:data.title,
            path:data.path,
        ...
        toast.success('Акцію добавлено в базу!');
        reset();
        props.closeModal();
    }
    return (
        <div className="relative w-1/4 h-screen  p-4 overflow-y-auto bg-gray-900">
            <h5 className="inline-flex items-center mb-6 text-sm font-semibold text-gray-500 uppercase w-full">Добавити акцію</h5>
            <button 
            type="button" 
            ...
            <form action="#" className="w-full" onSubmit={handleSubmit(addDiscountForm)}>
