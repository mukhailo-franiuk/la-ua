...
import { useAddVacancyMutation } from "../../../../store/vacanciesSlice/vacanciesSlice";
...
const Addvacancies = (props) => {
    ...
    const addVacancie = async (data) => {
       
        await addVacancy(data).unwrap();
        toast.success('Вакансію добавлено в базу!');
        reset();
        props.closeModal();
    }
    return (
        <div className="relative w-1/4 h-screen  p-4 overflow-y-auto bg-gray-900">
            <h5 className="inline-flex items-center mb-6 text-sm font-semibold text-gray-500 uppercase w-full">Добавити вакансію</h5>
            <button
                type="button"
            ...
            <form action="#" className="w-full" onSubmit={handleSubmit(addVacancie)}>
