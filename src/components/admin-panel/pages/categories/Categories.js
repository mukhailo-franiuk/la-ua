...
import { useGetCategoriesQuery, useDeleteCategoryMutation } from "../../../../store/categorySlice/categorySlice";
import AddCategory from "./AddCategories";
const Categories = () => {
    const [openOptionListId, setOpenOptionListId] = useState(null);
    const [isOpenAddCategoryForm, setIsOpenAddCategoryForm] = useState(false);
    const [isOpenQuestion, setIsOpenAddQuestion] = useState(false);
    const { data } = useGetCategoriesQuery();
    const [id, setId] = useState('');
    const closeWindowAddCategory = () => {
        setIsOpenAddCategoryForm(false);
    }
    ...
    return (
        <section className="bg-whitep-3 sm:p-5">
            <Toaster
                position="top-center"
                reverseOrder={false}
                gutter={8}
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                        ...
                        <div className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                            <button
                                ...
                                onClick={() => setIsOpenAddCategoryForm(true)}
                            >
                            ...
                            <div className="flex items-center space-x-3 w-full md:w-auto">
                                <button id="actionsDropdownButton" className="w-full md:w-auto flex items-center justify-center py-2 px-4 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700" type="button">
                            ...
                            <tbody>
                                {data?.slice(0, 10).map((item, index) => (
                                    <tr className="border-b border-white" key={item.id}>
                                        <th scope="row" className="px-4 py-3 font-medium text-white whitespace-nowrap">{index + 1}</th>
            ...
            <div className={`fixed z-10 w-full top-0 left-0 h-screen flex flex-col items-baseline  text-sm shadow-xl md:grid-cols-3 transition-all duration-700 ease-in-out ${isOpenAddCategoryForm ? "translate-x-0" : "-translate-x-full"}`}>
                <AddCategory closeModal={closeWindowAddCategory} />
            </div>
